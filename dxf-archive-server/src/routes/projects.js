...
const PROJECTS_MAX_TOTAL = 200; // 기본/검색 목록은 최대 이만큼만 모아서 그 안에서 페이지를 나눈다
    ...
    const result = await pool.query(
      `WITH capped AS (
       ...
       SELECT serial_no, project_name, operator_name, location, first_eia_year, agency_name,
        ...
        ORDER BY has_drawings DESC, sort_key DESC
        LIMIT $4 OFFSET $5`,
      [q, region?.province ?? null, region?.city ?? null, PROJECTS_PAGE_SIZE, offset]
    );

    const total = result.rows[0]?.total_count ?? 0;
    const projects = result.rows.map(({ total_count, ...rest }) => rest);
    res.json({ projects, total, page, pageSize: PROJECTS_PAGE_SIZE });
  } catch (e) {
    next(e);
  }
});
