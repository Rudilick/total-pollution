...
function createLifeModule(opts) {
  ...
  function _renderMethodCascade(m1, m2, m3, idPrefix) {
    ...
    let html = `
      <select class="miniSelect" style="min-width:160px;"
    ...
    if (m1 === "공공하수처리시설") {
      const opts = plants.length > 0
        ...
        : `<option value="">처리장 없음 (${sigun||"시군 선택 필요"})</option>`;
      html += `
        <select class="miniSelect" style="min-width:200px;"
      ...
      html += `
        <select class="miniSelect" style="min-width:120px;"
          onchange="window.__lifeOnMethod2Change('${rootId}','${idPrefix}',this.value)">
