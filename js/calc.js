...
function _rBOD(v){return _r(v,4);}
function _rTN(v){return _r(v,4);}
function _rTP(v){return _r(v,6);}
function _rFinalBOD(v){return _r(v,3);}
function _rFinalTN(v){return _r(v,3);}
function _rFinalTP(v){return _r(v,4);}
...
function _calcBizLoad(v,conc){
  ...
  const gen={BOD:_rBOD(v*conc.BOD/1000),TN:_rTN(v*conc.TN/1000),TP:_rTP(v*conc.TP/1000)};
  return{
    발생부하량:gen,
    분뇨발생부하량:{BOD:_rBOD(R.BOD*gen.BOD),TN:_rTN(R.TN*gen.TN),TP:_rTP(R.TP*gen.TP)},
    잡배수발생부하량:{BOD:_rBOD((1-R.BOD)*gen.BOD),TN:_rTN((1-R.TN)*gen.TN),TP:_rTP((1-R.TP)*gen.TP)},
  };
}
...
function _calcDirectTransfer(분뇨발생유량, method1, fecesFacility){
  ...
  const 방류부하량={
    BOD:_rBOD(직접이송유량*linkedPlant.efflBOD/1000),
    TN: _rTN(직접이송유량*(linkedPlant.efflTN||0)/1000),
    TP: _rTP(직접이송유량*linkedPlant.efflTP/1000),
  };
...
function _calcDischargeLoad(오수발생유량,발생부하량,method1,method2,method3,isWaterBuffer,plantInfo,fecesFacility,분뇨발생유량){
  if(method1==="공공하수처리시설"&&plantInfo){
    ...
    const 방류부하량={BOD:_rBOD(관거이송량*plantInfo.efflBOD/1000),TN:_rTN(관거이송량*(plantInfo.efflTN||0)/1000),TP:_rTP(관거이송량*plantInfo.efflTP/1000)};
    ...
    const 관거배출부하량={BOD:_rBOD(발생부하량.BOD*유실BOD),TN:_rTN(발생부하량.TN*유실TN),TP:_rTP(발생부하량.TP*유실TP)};
    return{
      배출부하량:{BOD:_rBOD(방류부하량.BOD+관거배출부하량.BOD),TN:_rTN(방류부하량.TN+관거배출부하량.TN),TP:_rTP(방류부하량.TP+관거배출부하량.TP)},
  ...
  if(method1==="개인하수처리시설"){
    ...
    const 개별배출부하량={BOD:_rBOD(잡배수유량*std.BOD/1000),TN:_rTN(잡배수유량*std.TN/1000),TP:_rTP(잡배수유량*std.TP/1000)};
...
function _calcLand(landObj){
  ...
  for(const[jmok,rawArea]of Object.entries(landObj)){
    ...
    const 발생부하량={BOD:_rBOD(unit.BOD*area/1e6),TN:_rTN(unit.TN*area/1e6),TP:_rTP(unit.TP*area/1e6)};
...
function runCalc(){
  ...
  const result={
    ...
    최종배출부하량:{
      점오염:{BOD:_rFinalBOD(lifeDelta.점오염.BOD),TN:_rFinalTN(lifeDelta.점오염.TN),TP:_rFinalTP(lifeDelta.점오염.TP)},
      비점오염:{BOD:_rFinalBOD(lifeDelta.비점오염.BOD+landDelta.BOD),TN:_rFinalTN(lifeDelta.비점오염.TN+landDelta.TN),TP:_rFinalTP(lifeDelta.비점오염.TP+landDelta.TP)},
    },
    단위유역별배출,
