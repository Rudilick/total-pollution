...
function _calcDirectTransfer(발생부하량, method1, fecesFacility){
  const coef=CALC_CONSTS.DIRECT_TRANSFER_COEF[method1];
  if(!coef||!fecesFacility) return null;

  // 직접이송 대상 부하량 = 발생부하량 × 직접이송대상계수 × 직접이송비(=1)
  const 직접이송부하량={
    BOD:_r(발생부하량.BOD*coef.BOD*CALC_CONSTS.DIRECT_TRANSFER_RATIO),
    TN: _r(발생부하량.TN *coef.TN *CALC_CONSTS.DIRECT_TRANSFER_RATIO),
    TP: _r(발생부하량.TP *coef.TP *CALC_CONSTS.DIRECT_TRANSFER_RATIO),
  };

  // 연계 하수처리장 방류농도 적용
  const linkedPlant=_getPlantInfo(fecesFacility.linkedPlant);
  if(!linkedPlant) return{직접이송부하량, 방류부하량:_zero(), 처리장:fecesFacility.linkedPlant, unitBasin:""};

  // 직접이송유량 = 발생부하량 유량 기준으로 추정 (단순화: 분뇨발생유량 비율 사용)
  // 실제로는 직접이송유량 = 오수발생유량 × 직접이송대상계수_유량
  // 여기서는 부하량 비율로 역산
  const 방류부하량={
    BOD:_r(직접이송부하량.BOD*(linkedPlant.efflBOD/(발생부하량.BOD>0?발생부하량.BOD*1000/1:1)) || 0),
    TN: 0,
    TP: _r(직접이송부하량.TP*(linkedPlant.efflTP/(발생부하량.TP>0?발생부하량.TP*1000/1:1)) || 0),
  };

  return{
    직접이송부하량,
    방류부하량: 직접이송부하량,  // 단순화: 직접이송부하량을 처리장 방류농도로 재계산 필요
...
function _calcDischargeLoad(오수발생유량, 발생부하량, method1, method2, method3, isWaterBuffer, plantInfo, fecesFacility){
  ...
  if(method1==="개인하수처리시설"){
    ...
    // 직접이송 계산
    const 직접이송결과=_calcDirectTransfer(발생부하량, method1, fecesFacility);
...
function _calcByUnitBasin(lifeAfter, lifeBefore, landAfter, landBefore, projectBasin){
  ...
  for(const basin of Object.keys(result)){
    for(const type of Object.keys(result[basin])){
      result[basin][type]={BOD:_r(result[basin][type].BOD),TN:_r(result[basin][type].TN),TP:_r(result[basin][type].TP)};
    }
  }
  return result;
}
