  ...
  function simpleTable(headers, rows, colWidths, { thick = false, headShade = "D9E2F3" } = {}) {
    ...
    const headerRow = new TableRow({
      ...
      children: headers.map((h, i) => tc(
        ...
        { w: colWidths[i], shading: headShade, borders: nb }
      )),
    });
...
function buildCoverSection(docx, data) {
  const { Paragraph, TextRun, Table, TableRow, TableCell,
          AlignmentType, UnderlineType, WidthType, BorderStyle, VerticalAlign } = docx;
...
function buildJeogamBlock(docx, data) {
  ...
  const table = new Table({
    ...
    rows: [
      ...
      new TableRow({ height:{value:ROW_H}, children:[
        cell(p("-"), cell(p("-")), cell(p("-")), cell(p("-")),
      ].flat().filter(x=>x instanceof TableCell) }),
      new TableRow({ height:{value:ROW_H}, children:[
        cell(p("-")), cell(p("-")), cell(p("-")), cell(p("-")),
      ]}),
    ]
...
function generateDoc(calcResult) {
  ...
  try {
    if (!window.docx) throw new Error("docx 라이브러리가 로드되지 않았습니다.");
    const { Document, Packer, SectionType } = window.docx;
    ...
    // ── 섹션 조립 ──────────────────────────────────────────────
    const coverSection = buildCoverSection(window.docx, data);

    const bodyChildren = [
      ...buildChongGwalBlock(window.docx, data),
      ...buildJeogamBlock(window.docx, data),
      ...buildLifeSection(window.docx, calcResult, envRiver),
      ...buildLandSection(window.docx, calcResult, unitBasin),
      ...buildFinalSection(window.docx, calcResult, unitBasin),
      ...buildMitigationSection(window.docx, data),
      ...buildAppendixSection(window.docx),
    ];
