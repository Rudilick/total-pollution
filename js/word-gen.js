// =========================================================
    ...
    function generateDoc(calcResult) {
      // calcResult 없으면 마지막 계산 결과 사용, 그것도 없으면 계산 먼저 실행
      if (!calcResult) {
      ...
      try {
          ...
          const headerRow = new TableRow({
            ...
            children: headers.map(h => new TableCell({
              ...
              children: [ new Paragraph({ alignment: AlignmentType.CENTER,
                children: [new TextRun({ text: h, font, size: 18, bold: true })] }) ]
            }))
          });
          ...
          const dataRows = rows.map(r => new TableRow({
            children: r.map((cell, ci) => new TableCell({
              ...
              children: [ new Paragraph({
                ...
                children: [new TextRun({ text: String(cell ?? ""), font, size: 18 })]
              }) ]
            }))
          }));
        ...
        function buildLifeTableRows(lifeSection) {
          if (!lifeSection) return [];
          const rows = [];
          const biz = lifeSection.영업인구?.rows || [];
          biz.forEach(r => {
            rows.push([
              `${r.buildingNo}동 ${r.floorNo}층`,
              r.mid + (r.minor ? `(${r.minor})` : ""),
              CalcFormat.area(r.적용면적) + (r.unitType === "area" ? "㎡" : "인"),
              r.오수발생원단위 + "ℓ/㎡",
              CalcFormat.flow(r.오수발생유량) + "㎥/일",
              CalcFormat.load(r.발생부하량.BOD),
              CalcFormat.load(r.발생부하량.TP),
              CalcFormat.load(r.배출부하량.BOD),
              CalcFormat.load(r.배출부하량.TP),
            ]);
          });
          // 합계 행
          const t = lifeSection.합계;
          rows.push(["합  계", "", "", "", CalcFormat.flow(t.오수발생유량),
            CalcFormat.load(t.발생부하량.BOD), CalcFormat.load(t.발생부하량.TP),
            CalcFormat.load(t.배출부하량.BOD), CalcFormat.load(t.배출부하량.TP)]);
          return rows;
        }

        function buildLandTableRows(landSection) {
          if (!landSection) return [];
          const rows = (landSection.rows || []).map(r => [
            r.jmok,
            CalcFormat.area(r.area) + "㎡",
            r.원단위.BOD + "",
            r.원단위.TP + "",
            CalcFormat.load(r.발생부하량.BOD),
            CalcFormat.load(r.발생부하량.TP),
          ]);
          const t = landSection.합계.발생부하량;
          rows.push(["합  계", "", "", "", CalcFormat.load(t.BOD), CalcFormat.load(t.TP)]);
          return rows;
        }

        const lifeHeaders = ["위치", "용도", "면적/인원", "오수원단위", "오수발생량(㎥/일)",
          "발생BOD(kg/일)", "발생T-P(kg/일)", "배출BOD(kg/일)", "배출T-P(kg/일)"];
        const landHeaders = ["지목", "면적(㎡)", "BOD원단위", "T-P원단위", "발생BOD(kg/일)", "발생T-P(kg/일)"];

        const cr = data.calcResult;
        const lifeBefore_rows = buildLifeTableRows(cr?.생활계?.사업전);
        const lifeAfter_rows  = buildLifeTableRows(cr?.생활계?.사업후);
        const landBefore_rows = buildLandTableRows(cr?.토지계?.사업전);
        const landAfter_rows  = buildLandTableRows(cr?.토지계?.사업후);

        // 결과 요약 행
        const summaryRows = cr ? [
          ["생활계 사업전 배출",
            CalcFormat.load(cr.생활계.사업전.합계.배출부하량.BOD),
            CalcFormat.load(cr.생활계.사업전.합계.배출부하량.TN),
            CalcFormat.load(cr.생활계.사업전.합계.배출부하량.TP)],
          ["생활계 사업후 배출",
            CalcFormat.load(cr.생활계.사업후.합계.배출부하량.BOD),
            CalcFormat.load(cr.생활계.사업후.합계.배출부하량.TN),
            CalcFormat.load(cr.생활계.사업후.합계.배출부하량.TP)],
          ["토지계 사업전 배출",
            CalcFormat.load(cr.토지계.사업전.합계.배출부하량.BOD),
            CalcFormat.load(cr.토지계.사업전.합계.배출부하량.TN),
            CalcFormat.load(cr.토지계.사업전.합계.배출부하량.TP)],
          ["토지계 사업후 배출",
            CalcFormat.load(cr.토지계.사업후.합계.배출부하량.BOD),
            CalcFormat.load(cr.토지계.사업후.합계.배출부하량.TN),
            CalcFormat.load(cr.토지계.사업후.합계.배출부하량.TP)],
          ["점오염 증가량(후-전)",
            CalcFormat.load(cr.최종배출부하량.점오염.BOD),
            CalcFormat.load(cr.최종배출부하량.점오염.TN),
            CalcFormat.load(cr.최종배출부하량.점오염.TP)],
          ["비점오염 증가량(토지계)",
            CalcFormat.load(cr.최종배출부하량.비점오염.BOD),
            CalcFormat.load(cr.최종배출부하량.비점오염.TN),
            CalcFormat.load(cr.최종배출부하량.비점오염.TP)],
        ] : [["계산 결과 없음","","",""]];
        // ─────────────────────────────────────────────────────────────
        ...
        const bodySection = {
          ...
          children: [
            ...
            makeCalcTable(docx, lifeHeaders, lifeBefore_rows),
            new Paragraph("표 2. 생활계 산정(후)"),
            makeCalcTable(docx, lifeHeaders, lifeAfter_rows),

            new Paragraph(""),
            new Paragraph({ text: "토지계 (사업시행 전/후)", heading: docx.HeadingLevel.HEADING_2 }),
            new Paragraph("표 3. 토지계 산정(전)"),
            makeCalcTable(docx, landHeaders, landBefore_rows),
            new Paragraph("표 4. 토지계 산정(후)"),
            makeCalcTable(docx, landHeaders, landAfter_rows),

            new Paragraph(""),
            new Paragraph({ text: "결과 요약", heading: docx.HeadingLevel.HEADING_2 }),
            new Paragraph("표 5. 전/후 비교 및 증감"),
            makeCalcTable(docx, ["구분","BOD(kg/일)","T-N(kg/일)","T-P(kg/일)"], summaryRows),
