...
function createLifeModule(opts) {
  ...
  function _renderMethodCascade(m1, m2, m3, idPrefix) {
    ...
    // ① 처리방법 선택 (항상 표시)
    html += `<select class="miniSelect" style="width:140px;flex-shrink:0;"
    ...
    if (m1 === "공공하수처리시설") {
      const opts = plants.length > 0
        ...
        : `<option value="">처리장 없음(${sigun||"시군 선택"})</option>`;
      html += `<select class="miniSelect" style="flex:1;min-width:0;max-width:200px;"
      ...
      html += `<select class="miniSelect" style="width:90px;flex-shrink:0;"
          onchange="window.__lifeOnMethod2Change('${rootId}','${idPrefix}',this.value)">
          <option value="고도처리" ${m2==="고도처리"?"selected":""}>고도처리</option>
          <option value="일반처리" ${m2==="일반처리"?"selected":""}>일반처리</option>
        </select>
        <select class="miniSelect" style="width:90px;flex-shrink:0;"
          onchange="window.__lifeOnMethod3Change('${rootId}','${idPrefix}',this.value)">
          <option value="50톤이상" ${m3==="50톤이상"||!m3?"selected":""}>50t 이상</option>
          <option value="50톤미만" ${m3==="50톤미만"?"selected":""}>50t 미만</option>
        </select>`;
    }
    // 정화조: 추가 없음
