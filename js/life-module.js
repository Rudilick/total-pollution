...
// life-module.js  v4
// [변경]
//   - UI 정비: 폰트 크기 통일, 드롭박스 너비 개선, 레이아웃 정돈
//   - 읍면지역 체크박스는 index.html에서 별도 관리 (window.isEupMyeon)
// ================================================================
...
function createLifeModule(opts) {
  ...
  function _renderFecesDrop(rootId, idPrefix, m4, sigun) {
    const flist = (typeof FECES_PLANT_DB !== "undefined")
      ? FECES_PLANT_DB.filter(p => p.sigun === sigun) : [];
    if (flist.length === 0) return `<span style="font-size:11px;color:#9ca3af;white-space:nowrap;">분뇨처리시설 없음</span>`;
    const opts = flist.map(p=>`<option value="${p.name}" ${m4===p.name?"selected":""}>${p.name}</option>`).join("");
    return `<select style="font-size:12px;min-width:160px;max-width:220px;"
  ...
  function _renderMethodCascade(m1, m2, m3, m4, idPrefix) {
    ...
    // ① method1
    html += `<select style="font-size:12px;min-width:140px;"
    ...
    if (m1 === "공공하수처리시설") {
      ...
      html += `<select style="font-size:12px;min-width:160px;max-width:240px;"
  ...
  function render() {
    ...
    const hhMethodHtml = _renderMethodCascade(
      state.householdMethod1, state.householdMethod2, state.householdMethod3, state.householdMethod4||"", "hh"
    );
    const hhMethodContainer = document.getElementById(`hhMethod_${rootId}`);
    if(hhMethodContainer) hhMethodContainer.innerHTML = hhMethodHtml;
      ...
      card.innerHTML = `
        <div class="buildingHead">
          ...
          <div style="display:flex;gap:6px;flex-shrink:0;">
            <button type="button" class="miniBtn"       data-act="addFloor"       data-b="${bIdx}">+ 층</button>
  ...
  function _renderUseRow(bIdx, fIdx, uIdx) {
    ...
    const nsHtml = `
      <div class="nsWrap" style="font-size:12px;">
        ...
        ${u.isNonSewage ? `
          <select class="miniSelect" style="font-size:11px;"
    ...
    return `
      <div class="${gridClass}" style="font-size:12px;">
        ...
        ${minorHtml}
        ${showInput ? `
          <input type="text" inputmode="decimal" value="${u.inputValue??""}" placeholder="면적/인원"
  ...
  function onMethod1Change(prefix, val) {
    if (prefix==="hh") { state.householdMethod1=val; state.householdMethod2=""; state.householdMethod3=""; state.householdMethod4=""; }
    else { const bIdx=Number(prefix.replace("b","")); state.buildings[bIdx].method1=val; state.buildings[bIdx].method2=""; state.buildings[bIdx].method3=""; state.buildings[bIdx].method4=""; }
    render();
  }
  function onMethod2Change(prefix, val) { if(prefix==="hh") state.householdMethod2=val; else { const bIdx=Number(prefix.replace("b","")); state.buildings[bIdx].method2=val; } }
  function onMethod3Change(prefix, val) { if(prefix==="hh") state.householdMethod3=val; else { const bIdx=Number(prefix.replace("b","")); state.buildings[bIdx].method3=val; } }
  function onMethod4Change(prefix, val) { if(prefix==="hh") state.householdMethod4=val; else { const bIdx=Number(prefix.replace("b","")); state.buildings[bIdx].method4=val; } }
  ...
  return { state, render, addBuilding, onCommonAreaChange, onMethod1Change, onMethod2Change, onMethod3Change, onMethod4Change,
