...
function createLifeModule(opts) {
  ...
  function _renderFecesDrop(rootId, idPrefix, m4, sigun) {
    const flist = (typeof FECES_PLANT_DB !== "undefined")
      ...
      : [];
    if (flist.length === 0) return `<span style="font-size:11px;color:#9ca3af;margin-left:4px;">분뇨처리시설 없음(사업개요 시군 확인)</span>`;
    const opts = flist.map(p=>`<option value="${p.name}" ${m4===p.name?"selected":""}>${p.name}</option>`).join("");
    return `<select class="miniSelect" style="width:190px;flex-shrink:0;"
        onchange="window.__lifeOnMethod4Change('${rootId}','${idPrefix}',this.value)">
        <option value="" disabled ${m4?"":"selected"} hidden>분뇨처리시설 선택</option>
        ${opts}
      </select>`;
  }
  ...
  function render() {
    ...
    // 가정인구 처리방법 컨테이너 업데이트 (index.html에 id="hhMethod_{rootId}" 필요)
  ...
  function onMethod4Change(prefix, val) {
    if (prefix==="hh") state.householdMethod4=val;
  ...
  function addBuilding() {
    ...
    state.buildings.push({ buildingNo:nextNo, method1:"공공하수처리시설", method2:"", method3:"", method4:"",
