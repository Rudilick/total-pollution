...
// [변경]
//   - 단위유역 드롭박스 (sigun 연동)
//   - index.html의 unitBasin input → unitBasinSelect 로 대응
//   - fillUnitBasinSelect() 추가
// ================================================================
...
function fillYearMonthSelects(){
  ...
  const ss=document.getElementById("startYearSelect"),cs=document.getElementById("completeYearSelect");
  if(!ys||!ms) return;
...
// ── 단위유역 드롭박스 (sigun 기반) ───────────────────────────
function fillUnitBasinSelect(){
  const sel=document.getElementById("unitBasinSelect"); if(!sel) return;
  const sigun=document.getElementById("sigunSelect")?.value||"";
  const list=typeof UNIT_BASIN_LIST!=="undefined"?UNIT_BASIN_LIST:[];
  const entry=list.find(e=>e.sigun===sigun);
  const basins=entry?.basins||[];
  const cur=sel.value;
  sel.innerHTML=`<option value="" disabled selected>단위유역 선택</option>`;
  basins.forEach(b=>{const o=document.createElement("option");o.value=b;o.textContent=b;sel.appendChild(o);});
...
function _showDetailedResults(result){
  ...
  const projectBasin=p.unitBasin||"-";

  // 기준 정보
  const infoHtml=`
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px 16px;margin-bottom:16px;font-size:13px;display:flex;gap:20px;flex-wrap:wrap;">
      ...
      <span><b>사업지 단위유역:</b> <b style="color:#7c3aed;">${projectBasin}</b></span>
      <span><b>수변구역:</b> <span style="color:${p.isWaterBuffer?"#dc2626":"#6b7280"};font-weight:900;">${p.isWaterBuffer?"해당":"해당없음"}</span></span>
    </div>`;

  // 단위유역별 배출 요약
  const byBasin=result.단위유역별배출||{};
  let basinHtml="";
  if(Object.keys(byBasin).length>0){
    basinHtml=`<div style="font-weight:900;font-size:13px;margin:0 0 6px;">단위유역별 최종 배출부하량 (후-전)</div>
    <table style="${tbl}margin-bottom:16px;">
      <tr><th style="${th}">단위유역</th><th style="${th}">배출구분</th><th style="${th}">BOD (kg/일)</th><th style="${th}">T-P (kg/일)</th></tr>`;
    for(const [basin,data] of Object.entries(byBasin)){
      if(data.점오염){ basinHtml+=`<tr><td style="${tdl}" rowspan="2" valign="top"><b>${basin}</b></td><td style="${td}">점오염</td><td style="${tds}">${f4(data.점오염.BOD)}</td><td style="${tds}">${f4(data.점오염.TP)}</td></tr>`; }
      if(data.비점오염){ basinHtml+=`<tr><td style="${td}">비점오염</td><td style="${td}">${f4(data.비점오염.BOD)}</td><td style="${td}">${f4(data.비점오염.TP)}</td></tr>`; }
    }
    basinHtml+=`</table>`;
  }
  ...
  function buildLifeSection(label,color,lifeData){
    ...
    if(hh){
      ...
      h+=`<table style="${tbl}"><tr><th style="${th}">인구수</th><th style="${th}">오수(㎥/일)</th><th style="${th}">발생BOD</th><th style="${th}">배출BOD</th><th style="${th}">발생T-P</th><th style="${th}">배출T-P</th></tr>
        ...
        ${hh.직접이송결과?`<tr><td style="${tdl}" colspan="2" style="font-size:11px;color:#6b7280;">└ 분뇨직접이송 → ${hh.직접이송결과.처리장||"-"}</td><td style="${td}">${f4(hh.직접이송결과.방류부하량?.BOD)}</td><td style="${td}">-</td><td style="${td}">${f4(hh.직접이송결과.방류부하량?.TP)}</td><td style="${td}">-</td></tr>`:""}
      </table>`;
    }
    ...
    if(bRows.length>0){
      ...
      bRows.forEach(r=>{
        ...
        const m=r.sewageMethod1==="공공하수처리시설"?`공공(${r.plantName||"미선택"})`:r.sewageMethod1;
        h+=`<tr><td style="${tdl}">${loc}</td><td style="${tdl}">${use}</td><td style="${td}">${area}</td>
          ...
          <td style="${td}">${f4(r.배출부하량?.BOD)}</td><td style="${td}">${f4(r.배출부하량?.TP)}</td><td style="${tdl}">${m}</td></tr>`;
      });
      h+=`</table></div>`;
    }
    const t=lifeData?.합계;
    if(t){ h+=`<table style="${tbl};margin-top:10px;"><tr>
      <th style="${th}">합계</th><th style="${th}">오수(㎥/일)</th>
      <th style="${th}">발생BOD</th><th style="${th}">배출BOD</th>
      <th style="${th}">발생T-P</th><th style="${th}">배출T-P</th></tr>
  ...
  function buildLandSection(label,color,landData){
    ...
    h+=`<table style="${tbl}"><tr><th style="${th}">지목</th><th style="${th}">면적(㎡)</th><th style="${th}">BOD원단위</th><th style="${th}">T-P원단위</th><th style="${th}">발생BOD</th><th style="${th}">발생T-P</th></tr>`;
    ...
    h+=`<tr><td style="${tds}" colspan="4">합계</td><td style="${tds}">${f4(t.BOD)}</td><td style="${tds}">${f4(t.TP)}</td></tr></table></div>`;
    return h;
  }
  ...
  el.innerHTML = infoHtml + basinHtml
    + `<div style="font-weight:900;font-size:13px;margin:0 0 6px;">생활계</div>`
    + `<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">`
    + buildLifeSection("▶ 사업전","#6b7280",result.생활계.사업전)
    + buildLifeSection("▶ 사업후","#2563eb",result.생활계.사업후)
    + `</div>`
    + `<div style="font-weight:900;font-size:13px;margin:0 0 6px;">토지계 (비점)</div>`
    + `<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">`
