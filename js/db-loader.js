...
const DB_EXCEL_URL = (typeof CONFIG !== "undefined") ? CONFIG.DB_EXCEL_URL : "";
    ...
    function inferUnit(unitRaw) {
      ...
      if (u.includes("㎡") || s.includes("m²") || s.includes("m2") || u.includes("제곱")) return { unitType: "area", unitText: u };
      if (u.includes("인") || u.includes("명") || s.includes("person")) return { unitType: "person", unitText: u };
      return { unitType: "custom", unitText: u };
    }

    // ── 대/중/소 계층 DB 빌드 ──
    // 규칙: 소분류가 하나라도 있으면 중분류는 terminal=false (소분류 select 표시)
    //       소분류가 공란이면 중분류가 최종(terminal=true)
    function buildLifeDBFromRows(rows) {
      ...
      for (const it of (rows || [])) {
        ...
        const minorSlim = minor.replace(/\s/g, "");
        if (minorSlim === "-" || minorSlim === "–" || minorSlim === "—") minor = "";
        ...
        if (minor) {
          ...
          if (node.terminal) {
            node.unitType = uinfo.unitType;
            node.unitText = uinfo.unitText;
          }
        }
      }
      return db;
